"""
runMonteParallel function with multiprocessing (using runSimulationParallel)
"""

from runSimulationParallel import runSimulationParallel
import math
...
def runMonteParallel(loanPool, structuredSecurit, tolerance, NSIM, numProcesses):
    """
    parameter: a LoanPool object, a StructuredSecurities object, tolerance, the number of simulations to run,
    number of precesses
    runMonteParallel function loop through runSimulationParallel(with multiprocessing)
    and adjust the tranche rates to reflect the yields
    ...
    while diff >= tolerance:
        ...
        swf = runSimulationParallel(loanPool, structuredSecurit, NSIM, numProcesses)
